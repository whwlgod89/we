...
//    private int mRssi;                  // User Defined RSSI Value (75, 85, 100)
...
//    private SensorDatabase mDB;
    ...
    public Sensor(Context context, /*ConnectionEvent CE, SensorDatabase db,*/ String sensorId, String sensorName, String phoneNumber, int actionMode, int rssi) {
...
//    public void setBatteryLevel(int batteryLevel) { mBatteryLevel = batteryLevel; }
    ...
    private final void rxHandler(BluetoothGattCharacteristic characteristic) {
        ...
        Log.w(TAG, "rxHandler: ID= "+ mSensor.getId() + ", CMD=" + cmd + ", DATA=" + data);
        ...
        switch(cmd) {
            case 0xDD:      // 센서에서 휴대폰 찾기 알람 시작 (data = 배터리 잔량)
                Log.w(TAG, "Start Alarm of Phone: ID= "+ mSensor.getId() + ", Battery=" + data);
            ...
            case 0xEE:      // 센서에서 휴대폰 찾기 알람 종료 (data = 배터리 잔량)
                Log.w(TAG, "Stop Alarm of Phone: ID= "+ mSensor.getId() + ", Battery=" + data);
            ...
            case 0xF0:      // 센서 단말기의 FW 버전 정보 (data = 버전 정보)
                Log.w(TAG, "Response of Firmware: ID= "+ mSensor.getId() + ", Version=" + data);
                break;
            case 0xF1:      // 센서에서 주기적으로 발신하는 배터리 잔량 (data = 배터리 잔량)
                Log.w(TAG, "Report of Battery Level: ID= "+ mSensor.getId() + ", Battery=" + data);
                mSensor.setBattery(data);
                updateUI(Const.ACTION_SENSOR_BATTERY);
                //battery = true;
                break;
            case 0xFF:      // 센서 단말기의 전원 OFF 알림 (data = 배터리 잔량)
                Log.w(TAG, "rxHandler(): Report of Sensor OFF: ID= "+ mSensor.getId() + ", Battery=" + data);
                //disconnected = true;
...
//        }
    ...
    private final BluetoothGattCallback mGattCallback = new BluetoothGattCallback() {
        ...
        public void onConnectionStateChange(BluetoothGatt gatt, int status, int newState) {
            Log.i(TAG,"ID: "+ mSensor.getId() + " onConnectionStateChange(), newState=" + newState + ", mConnectState=" + mSensor.getState());

            if (newState == BluetoothProfile.STATE_CONNECTED) {
                Log.i(TAG,"ID: "+ mSensor.getId() + " Connected to GATT server.");
            ...
            // 연결이 끊어진 상태에서도 DISCONNECTED가 오는 경우가 있다. 현재 상태 체크를 추가한다.
                ...
                Log.i(TAG,"ID: " + mSensor.getId() + " Disconnected from GATT server.");
        ...
        public void onServicesDiscovered(BluetoothGatt gatt, int status) {
            if (status == BluetoothGatt.GATT_SUCCESS) {
                Log.w(TAG, "mBluetoothGatt.onServicesDiscovered() - ID: " + mSensor.getId() + ", mConnectState=" + mSensor.getState());
                ...
                Log.w(TAG, "ID: "+ mSensor.getId() + " onServicesDiscovered received: " + status);
            }
        }
        ...
        public void onReadRemoteRssi(BluetoothGatt gatt, int rssi, int status) {
            Log.d(TAG, "onReadRemoteRssi: Sensor=" + mSensor.getName() + ", RSSI=" + rssi + ", STATUS=" + status);
            if(status == BluetoothGatt.GATT_SUCCESS) {
                if(mSensor.getMode() == Const.ACTION_MODE_THEFT && rssi < Const.ACTION_THEFT_ALARM_RSSI) {
    ...
    public void readRemoteRSSI() {
        if(mBluetoothGatt != null) {
            Log.d(TAG, "readRemoteRssi: Sensor=" + mSensor.getName());
...
//    };
    ...
    private Runnable rxSensorInit = new Runnable() {
        public void run() {
            if(mQueryCount > 3) {
                ...
                Log.d(TAG, "rxSensorInit(): ID=" + mSensor.getId() + ", ConnectState=" + mSensor.getState());
                sensor_init();
            }
        }
    };
    ...
    public void sensor_init() {
        Log.d(TAG, "sensor_init(): ID=" + mSensor.getId() + ", ConnectState=" + mSensor.getState());
        if(mSensor.getState() != CONNECT_STATE.CONNECTED.ordinal()) {
            return;
        }
    ...
    public void findSensor() {
        Log.d(TAG, "findSensor(): ID=" + mSensor.getId() + ", ConnectState=" + mSensor.getState()+ ", FindState=" + mFindingSensor);
        if(mSensor.getState() != CONNECT_STATE.CONNECTED.ordinal()) {
...
//    }
    ...
    public boolean connect() {
        Log.d(TAG, "ID: "+ mSensor.getId() +" connect() called. mConnectState=" + mSensor.getState());
        ...
        if(mSensor.getState() == CONNECT_STATE.CONNECTED.ordinal()) {
            ...
            Log.w(TAG, "ID: " + mSensor.getId() +" Already connected.");
            return false;
        }
        ...
        if (mBluetoothGatt != null) {
            Log.w(TAG, "connect().mBluetoothGatt.connect() - ID: "+ mSensor.getId() + ", mConnectState=" + mSensor.getState());
            if (mBluetoothGatt.connect()) {
                ...
                Log.w(TAG, "ID: "+ mSensor.getId() + " Connected OK!");
                return true;
            } else {
                Log.w(TAG, "ID: "+ mSensor.getId() + " Connect FAILED!");
                return false;
            }
        }
        ...
        if (device == null) {
            ...
            Log.w(TAG, "ID:" + mSensor.getId() + " Device not found.  Unable to connect.");
            return false;
        }

        // We want to directly connect to the device, so we are setting the autoConnect
        // parameter to false.
        Log.d(TAG, "connect().device.connectGatt() - ID: "+ mSensor.getId() + ", mConnectState=" + mSensor.getState());
        mBluetoothGatt = device.connectGatt(mContext, false, mGattCallback);
        if(mBluetoothGatt != null) {
            Log.w(TAG, "ID: "+ mBluetoothGatt.getDevice().getAddress() + " connectGatt() OK!");
        }
        else {
            Log.w(TAG, "ID: "+ mSensor.getId() + " connectGatt() FAILED");
        }
        return true;
    }
    ...
    public void disconnect() {
        ...
        if (mBluetoothAdapter == null || mBluetoothGatt == null) {
            ...
            Log.w(TAG, "ID: "+ mSensor.getId() + " BluetoothAdapter not initialized");
            return;
        }
    ...
    public void disconnect(boolean force) {
        ...
        Log.w(TAG, "disconnect(): Sensor OFF ID= "+ mSensor.getId() );
        disconnect();
    }
...
//    };
    ...
    private Runnable reconnect = new Runnable() {
        public void run() {
            Log.w(TAG, "ID:" + mSensor.getId() + " reconnect()");
            if(mBluetoothGatt != null && mSensor.getState() != CONNECT_STATE.CONNECTED.ordinal()) {
                ...
                //connect();
                if(mBluetoothGatt.connect()) {
                    Log.w(TAG, "ID:" + mSensor.getId() + " reconnect() - mBluetoothGatt.connect() OK!");
                }
                else {
                    Log.w(TAG, "ID:" + mSensor.getId() + " reconnect() - mBluetoothGatt.connect() FAILED!");
                }

                mSensor.setState(CONNECT_STATE.CONNECTING.ordinal());
            }else {
                Log.w(TAG, "ID:" + mSensor.getId() + " reconnect() - mBluetoothGatt == NULL !!!!!!!");
                //connect();
            }
...
//    };
    ...
    private Runnable serviceDiscoveredFailed = new Runnable() {
        @Override
        public void run() {
            Log.w(TAG, "ID: "+ mSensor.getId() + " serviceDiscoveredFailed()");
            if(mSensor.getState() != CONNECT_STATE.CONNECTED.ordinal()) {
                //disconnect(true);
                mAutoHandler.postDelayed(reconnect, 100);
            }
    ...
    public void close() {
        ...
        Log.w(TAG, "ID: "+ mSensor.getId() + " mBluetoothGatt closed");
        mSensor.setId("");          //FIXME
        mBluetoothGatt.close();
        mBluetoothGatt = null;
    ...
    {
        if (mBluetoothGatt == null) {
            Log.d(TAG, "writeRXCharacteristic() - BluetoothGatt = NULL.. ID=" + mSensor.getId());
            return;
        }
        Log.d(TAG, "writeRXCharacteristic() - BluetoothGatt=" + mBluetoothGatt + ", ID=" + mSensor.getId());
        ...
        Log.d(TAG, "ID: " + mSensor.getId() +  " write TXchar - status=" + status);
    }
    ...
    public void actionUI(String action) {
        ...
        Log.d(TAG, "ID: "+ mSensor.getId() + " actionUI() called.");
    ...
    public void initialize() {
        ...
        Log.d(TAG, "ID:" + mSensor.getId() + " initialize() called.");
